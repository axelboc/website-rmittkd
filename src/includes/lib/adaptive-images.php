...
$resolutions   		= array(960, 800, 640, 480); // the resolution break-points to use (screen widths, in pixels)
$cache_path    		= "images/cached"; // where to store the generated re-sized images. Specify from your document root!
...
$width_coeff 		= 1; // multiple the width of the image by this coefficient when checking against the resolution - images are never as wide as the screen.
...
function generateImage($source_file, $cache_file, $resolution) {
  global $sharpen, $jpg_quality, $width_coeff;
  ...
  if ($width * $width_coeff <= $resolution) { // no, because the width of the source image is already less than the client width
    return $source_file;
  }
